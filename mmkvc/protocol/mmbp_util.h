...
#define MMKV_PARSE_UNSIGNED_(bits_)                                            \
  ...
  (void)(err);                                                                 \
  if (err != KVARINT_OK) return false;                                         \
...
#define DEF_PARSE_UNSIGNED_RAW_(bits_)                                         \
  MMKV_INLINE bool ParseComponent(uint##bits_##_t &i, void const **pp_data,    \
                                  size_t *p_size)                              \
  {                                                                            \
...
#define DEF_PARSE_SIGNED_RAW_(bits_)                                           \
  MMKV_INLINE bool ParseComponent(int##bits_##_t &i, void const **pp_data,     \
                                  size_t *p_size)                              \
  {                                                                            \
...
MMKV_INLINE bool ParseComponent(uint8_t &i, Buffer &buffer)
{
  MMKV_PARSE_UNSIGNED_(8);
}

MMKV_INLINE bool ParseComponent(uint16_t &i, Buffer &buffer)
{
  MMKV_PARSE_UNSIGNED_(16);
}

MMKV_INLINE bool ParseComponent(uint32_t &i, Buffer &buffer)
{
  MMKV_PARSE_UNSIGNED_(32);
}

MMKV_INLINE bool ParseComponent(uint64_t &i, Buffer &buffer)
{
  MMKV_PARSE_UNSIGNED_(64);
}

MMKV_INLINE bool ParseComponent(int64_t &i, Buffer &buffer)
{
...
MMKV_INLINE void
ParseComponent(std::basic_string<char, std::char_traits<char>, Alloc> &str,
               void const **pp_data, size_t *p_size, bool is_16 = false)
{
...
MMKV_INLINE void ParseComponent(
    std::vector<std::basic_string<char, std::char_traits<char>, Alloc>> &values,
    void const **pp_data, size_t *p_size)
{
  PREPARE_RAW_VAR_;
  uint32_t count = -1;
...
MMKV_INLINE void ParseComponent(WeightValues &values, void const **pp_data,
                                size_t *p_size)
{
  PREPARE_RAW_VAR_;
  uint32_t count = -1;
...
MMKV_INLINE void ParseComponent(StrKvs &kvs, void const **pp_data,
                                size_t *p_size)
{
  PREPARE_RAW_VAR_;
  uint32_t count = -1;
...
MMKV_INLINE void ParseComponent(uint8_t *u8s, size_t len, void const **pp_data,
                                size_t *p_size)
{
  for (size_t i = 0; i < len; ++i) {
...
MMKV_INLINE void ParseComponent(std::vector<char> &buf, void const **pp_data,
                                size_t *p_size)
{
  PREPARE_RAW_VAR_;
  uint32_t len = -1;
...
#define MMKV_SERIALIZE_UNSIGNED_(bits_)                                        \
  ...
  LOG_DEBUG << "unsigned varint buf size = " << buf.len

#define MMKV_SERIALIZE_SIGNED_(bits_)                                          \
  kvarint_buf##bits_##_t buf;                                                  \
  kvarint_encode##bits_##s(i, &buf);                                           \
  buffer.Append(buf.buf, buf.len);                                             \
  LOG_DEBUG << "signed varint buf size = " << buf.len
