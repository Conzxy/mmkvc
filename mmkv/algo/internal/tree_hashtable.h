 ...
 * class HashTable : public HashTableBase <K, V, GK, LinkedList<K, V, GK, EK,
 * Alloc>, Alloc> { ...
 * }; template<typename K, typename V, typename GK, typename Comparator,
 * typename Alloc> class TreeHashTable : public HashTableBase <K, V, GK, Tree<K,
 * V, GK, Comparator, Alloc>, Alloc> { ...
 * };
 * 同时，这也避免了HashTable的具体实现需要使用predicate，因为LT都有GK，但这其实也有个假设就是EK
 * == empty class，不然space cost 会比较大(8bytes/list)。
...
/**
 * \brief Hash table implemented by separate-list method but the list is avltree
 ...
 */
template <typename K, typename V, typename HF, typename GK, typename Tree,
          typename Alloc>
class TreeHashTable
  : protected Alloc::template rebind<typename Tree::Node>::other
  , protected HF
  , protected GK {
 ...
 private:
  using NodeAllocTraits =
      std::allocator_traits<typename Alloc::template rebind<Slot>::other>;
 ...
 public:
   ...
   * \param duplicate inserted new entry(success), duplicated entry with same
   * key(failure) \return true -- success false -- failure
   */
  bool InsertWithDuplicate(value_type const &elem, value_type *&duplicate)
  {
    return InsertWithDuplicate_impl(elem, duplicate);
  }
  bool InsertWithDuplicate(value_type &&elem, value_type *&duplicate)
  {
  ...
  {
    return const_cast<TreeHashTable *>(this)->Find(key);
  }
   ...
   *   satisfied node
   * \note
   *   If you want do something to the bucket, for example, do something then
   * delete it, get the bucket can decrease calculate the hash value again
   */
  Node *FindNode(K const &key, Bucket **bucket);
  Node const *FindNode(K const &key) const
  {
    return const_cast<TreeHashTable *>(this)->FindNode(key, nullptr);
  }
   ...
   * \return
   *   The number of erased entry
   *   In fact, if node is not null pointer and is valid node returned by
   * another interface it must return 1 instead of 0(former case) or
   * undefined(latter case)
   */
   ...
   *   Indeed, using enable_if_t and is_trivially_destructive can take static
   * check here but I don't like use SFINAE traits in such way.
   */
  ...
  {
    return ((double)tables_[0].used) / tables_[0].size;
  }
 ...
 private:
  ...
  {
    return const_cast<TreeHashTable *>(this)->table1();
  }
  Table &table2() noexcept { return tables_[1]; }
  Table const &table2() const noexcept
  {
    return const_cast<TreeHashTable *>(this)->table2();
  }
