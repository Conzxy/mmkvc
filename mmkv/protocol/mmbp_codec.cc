...
void MmbpCodec::SetUpConnection(TcpConnectionPtr const &conn)
{
  conn->SetMessageCallback(
      [this](TcpConnectionPtr const &conn, Buffer &buffer, TimeStamp recv_time) {
        ...
        while (true) {
          const auto decode_ret = kvarint_decode32(
              buffer.GetReadBegin(),
...
MmbpCodec::ErrorCode MmbpCodec::Parse(Buffer &buffer, MmbpMessage *message)
{
  ...
  if (KVARINT_OK == decode_ret) {
    ...
    if (buffer.GetReadableSize() < MIN_SIZE || buffer.GetReadableSize() >= MAX_SIZE) {
