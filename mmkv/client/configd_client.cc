...
ConfigdClient::~ConfigdClient() noexcept
{
  LOG_DEBUG << "Configd Client is destroyed";
}
...
void ConfigdClient::OnMessage(const TcpConnectionPtr &conn, Buffer &buffer,
  ...
  switch (resp.status()) {
    case CONF_STATUS_OK: {
      ...
      for (auto const &id_node_conf : node_conf_map) {
        ...
        NodeEndPoint node_end_point{(node_id_t)id_node_conf.first,
                                    node_conf.host(),
                                    (uint16_t)node_conf.port()};

        for (auto const shard_id : shard_ids) {
          auto ok = shard_endpoint_dict_.InsertKvWithDuplicate(
              (shard_id_t)shard_id, std::move(node_end_point), p_dep_key_value);
...
MMKV_INLINE static void AppendShardInterval(std::string &shard_str,
                                            shard_id_t &begin, shard_id_t &end)
{
  shard_str += '[';
